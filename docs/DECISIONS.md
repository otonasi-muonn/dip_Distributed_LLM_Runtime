...
**残っているのは別の問題**: 実 MoE モデルを WebGPU peer で走らせると生成が返らない (O11)。カーネルの正しさでも単純な遅さでも説明が付いておらず、しかも**測定は統合GPU 1 台でしか行っていない**。実 peer 機での再現確認が先。
