...
| **O11** | **実 MoE モデルの WebGPU peer 実行で、生成開始後 4.4 分 (覚醒確認済み) 経っても first token に到達しない。** CPU peer では同じモデルが 30 秒未満で完走する。**WebGPU 固有経路まで切り分け済みだが、hang / 極端な同期遅延 / 特定 op・graph 構成のどれかは未確定。** 対照実験で言えるのは「問題領域が **WebGPU backend 固有、または WebGPU backend と実 MoE graph / RPC の組み合わせ**」までで、RPC serialization 単体は CPU peer で動くものの「RPC で作られた graph × WebGPU の buffer / layout / command encoding」の相互作用は残る。⚠️ **カーネルの数値的な正しさは否定されている** — `test-backend-ops` で `MUL_MAT_ID` は WebGPU 455/455 一致 (F46)。⚠️ **「単純な遅さではない」とは言えない** — 単体 op が 3.4-5.1 ms/dispatch でも、graph 全体 (routing / copies / 同期 / RPC / queue submit-wait) の所要時間は導けない。⚠️ **GPU 利用率 5.8% からも何も断定できない** — 同期ボトルネックでも普通に出るし、GPU が動いていることは graph が前進している証明にもならない。**機構仮説 (未確認)**: `ggml-webgpu.cpp:505-520` の `WaitAny` は `WEBGPU_WAIT_ANY_TIMEOUT_MS = 100` で回り、`handle_wait_status(status, true)` が false のとき `subs` を消さずに再ループする。`TimedOut` は `allow_timeout` 時に無言で false、`Error` も log 後に false なので、**どちらでもエラー無しに永久リトライしうる** | ① `graph_compute` の**進行位置**を出す trace を入れる (encode 中 / submit 後 / partial wait / final wait / `WaitAny` の status)。② **discrete GPU 機で再現するか確かめる** — 測定機は AMD Radeon 780M (統合GPU) 1 台のみで、機種固有の可能性を潰していない。⚠️ upstream `a95a11e5b` (#22464、`MUL_MAT_ID` の性能改善) は候補だが **効くかは未判定**。進行位置を特定する前に入れない | **P0** |
