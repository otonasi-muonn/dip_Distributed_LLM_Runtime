...
| # | 事実 | 対象実装 |
|---|---|---|
| **F39** | **llama.cpp RPC client は RPC 操作ごとに socket を開閉する。** 1 回のモデルロード + 1 回の生成で **peer が accept した論理接続は 175 本** (in-app pane と実 Chrome で同値を再現)。同時に live なのは常に 1 本 (open → close → open の順で、重ならない)。⚠️ **Web repo の `FD_MAX = 1024` は約 6 世代で一周する**ので、fd 番号の再利用は遠い仮定ではなく通常運用で起きる。llmlet のコメント "the C code does frequent close and open of the socket" はこの意味だった | 実測 (Runtime-only harness) |
| **F40** | **`patches/0001` / `0002` は実行時に効いている。in-app pane と実 Chrome の両方で確認。** peer 側 `fdmax=4` で 1 セッションを回すと、fd 0-3 が **それぞれ 43-44 回再利用**され、最終値は **accepted=175 / registrations=175 / runtimeCloses=175 (lag 0)**。`runtimeCloses == accepted` は **0002** (RPC server が accept した fd を閉じる)、`registrations == accepted` は **0001** (`close_peer` が `_connbuf` slot を消すので `recv_peer` が再登録する) の直接証拠。⚠️ **ptr は毎回同じ値 (`102629376`) が返った** — `free` 直後の `malloc` が同じアドレスを返すので、**ptr の一致/不一致は判定に使えない** | 実測 (Runtime-only harness) |
...
| **F42** | **O9 の正体は WebGPU buffer の二重破棄で、force-exit 固有ではない。** requester の graceful stop (空 prompt → `main()` return → `llama_free`) でも再現し、スタックは `WGPUBufferImpl::Destroy()` → `_emwgpuBufferDestroy` → `WebGPU.getJsObject` → **`assert(ptr in WebGPU.Internals.jsObjects)` 失敗** → `abort("Assertion failed")` → `___trap()` → `RuntimeError: unreachable`。⚠️ **`stopTimeoutMs = 120000` にして force-exit fallback が発火しないことを確認したうえで再現**したので、adapter の fallback が原因ではない。`~llama_context` がバッファ解放を完了したログの直後に起きる。**adapter は `onError` で正しく通知し、`stop()` は resolve し、peer は無傷で、次の requester は正常に接続・生成できる**。⚠️ **実 Chrome では stop を未実行**なので、この節は in-app pane の実測のみに基づく | 実測 (Runtime-only harness) |
...
| **O9** | ⚠️ **2026-08-26 に原因特定 (F42)。`/restart` 固有ではなく graceful stop でも起きる WebGPU buffer の二重破棄。** 以下は特定前の記述。**`/restart` で WebGPU cleanup が落ちる可能性。** 2026-08-25 の 2PC 試行中に `/restart` を実行したところ `RuntimeError: unreachable` → `WGPUBufferImpl::Destroy()` → `webgpu_buf_pool::cleanup()` で停止した。**DataChannel の失敗とは無関係の別事象**。`webgpu_buf_pool::cleanup` は実在し (`.work/llmlet/llama.cpp/ggml/src/ggml-webgpu/ggml-webgpu.cpp:193-205`)、`/restart` は `runningClient.exit()` → `Module._emscripten_force_exit(0)` を呼ぶ (`llmlet.js:945-951`) | **段階A (同一PC) で単独に、最大2回・時間を区切って**試す。⚠️ **段階B (物理2PC) では `/restart` を使わない** — O4 の実験と O9 の再現が混ざるため。段階B の復帰は reload → Peer ID 再入力。⚠️ **非再現時の結論は「final SHA / Chrome 151 / 同一PC 3タブ で N 回中 0 回、未再現」までに留める。** 「旧バンドル固有」とは書かない — それを言うには旧版で再現→新版で十分回数の非再現が要る | P2 |
