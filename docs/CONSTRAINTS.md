...
| **F24** | **plain `http://` の LAN IP では Runtime が起動できない。実測済み。** `http://192.168.0.26:8889` で COOP/COEP を正しく送っていても `crossOriginIsolated: false`、`SharedArrayBuffer: undefined`、`navigator.gpu: undefined` になる。Chrome のコンソールは明示的に `The Cross-Origin-Opener-Policy header has been ignored, because the URL's origin was untrustworthy. Please deliver the response using the HTTPS protocol. You can also use the 'localhost' origin instead.` と出す。**ヘッダの問題ではなく origin の問題なので、サーバ設定では直せない** | 実測 (Chrome / 2026-08-23) |
| **O8** | **証明書を参加者端末にどう信頼させるか。** F24 より複数PC構成には TLS が必須。自己署名証明書でサーバを立てること自体は確認済み (`scripts/serve-runtime.py --cert/--key`、curl で 200) だが、**信頼していないブラウザは証明書警告で止まる** (本セッションの検証ブラウザは警告を通過させずナビゲーション自体を拒否した)。チーム自身の数台なら証明書を入れれば済むが、**QRコードで不特定の参加者端末を集める構成とは両立しない** | 会場で使う端末の範囲を決める。BYOD を諦めるか、正当に信頼される証明書を用意するか | **P0 (製品判断)** |
