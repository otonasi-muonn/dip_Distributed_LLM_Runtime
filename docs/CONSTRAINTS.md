...
| **O11** | **MoE グラフが WebGPU peer で返らない。** granite 1B-A400M (32 experts) を WebGPU peer で走らせると、load・graph 構築・`ready` までは通るのに生成が 4.4 分 (覚醒確認済み) 経っても 1 トークンも出ない。peer 側の RPC コマンドは 14 件で停止し、**requester 側にも peer 側にもエラーが出ない**。GPU 利用率は 5.8% で、hang でも飽和でもない。⚠️ **カーネルの正しさの問題ではない** — `test-backend-ops` で `MUL_MAT_ID` は WebGPU 455/455 一致 (F46)。⚠️ **単純な遅さでも説明できない** — 単体性能は 128 experts / k=2048 で 3.4-5.1 ms/dispatch であり、72 dispatch/token なら数百 ms のはず。対照実験で**モデル・グラフ・RPC・adapter は否定済み** (CPU peer なら同じモデルが 30 秒未満で完答、dense なら同じ WebGPU peer が数十秒で完答) | **まず実 peer 機 (VRAM 8GB 以上の discrete GPU) で再現するか確かめる。** 測定機は AMD Radeon 780M (統合GPU) 1 台であり、これが機種固有の問題である可能性を潰していない。再現したら「1 回の `graph_compute` が長い」のか「待って進まない」のかを決める — そのためには `graph_compute` の進行位置を出す仕掛けが要る。⚠️ upstream `a95a11e5b` (#22464、`MUL_MAT_ID` の性能改善) は候補だが、**数値上「遅さ」だけでは説明が付かないので、これで直ると仮定して先に入れないこと** | **P0** |
