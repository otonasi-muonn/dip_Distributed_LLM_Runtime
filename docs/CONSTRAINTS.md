...
| **O9** | **`/restart` で WebGPU cleanup が落ちる可能性。** 2026-08-25 の 2PC 試行中に `/restart` を実行したところ `RuntimeError: unreachable` → `WGPUBufferImpl::Destroy()` → `webgpu_buf_pool::cleanup()` で停止した。**DataChannel の失敗とは無関係の別事象**。`webgpu_buf_pool::cleanup` は実在し (`.work/llmlet/llama.cpp/ggml/src/ggml-webgpu/ggml-webgpu.cpp:193-205`)、`/restart` は `runningClient.exit()` → `Module._emscripten_force_exit(0)` を呼ぶ (`llmlet.js:945-951`) | **段階A (同一PC) で単独に、最大2回・時間を区切って**試す。⚠️ **段階B (物理2PC) では `/restart` を使わない** — O4 の実験と O9 の再現が混ざるため。段階B の復帰は reload → Peer ID 再入力。⚠️ **非再現時の結論は「final SHA / Chrome 151 / 同一PC 3タブ で N 回中 0 回、未再現」までに留める。** 「旧バンドル固有」とは書かない — それを言うには旧版で再現→新版で十分回数の非再現が要る | P2 |
