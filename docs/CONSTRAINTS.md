...
| # | 事実 | 対象実装 |
|---|---|---|
| **F56** | **O11 の停止候補が encode ループ内にある。** granite 1B-A400M を WebGPU peer で走らせ `GGML_WEBGPU_TRACE=1` (patches/0005) を有効にした旧 build では、peer の trace は **10 行で停止**した: `graph_compute begin n_nodes=1591` → `encode node=0 op=SCALE` → `submit #0 after node=48 kernels=33` → `wait(partial) end retries=0` → `encode node=64 op=ADD` → `submit #1 after node=104 kernels=32` → `wait(partial) end retries=0` → **`encode node=128/1591 op=ADD` (最終行)**。生成開始から 678 秒後も同一。⇒ ①**最後の trace event は encode** で `wait(final)` に到達していない ②`retries=0` は blocking wait loop が 0 回だったことを示すだけで、当時の trace は既存の 0-timeout poll の status / `subs` 変化を記録していないため、投入済み GPU work の完了は判定できない ③blocking `WaitAny` の retry trace が無いことから、少なくともこの経路で retry が観測されたとは言えないが、全ての `WaitAny` retry 仮説を反証はしない。encode ログは 64 node 間隔・submit は 32 kernel 毎なので、この旧測定の停止範囲は **node 129..192 のどこか** (`submit #2` は出ない) | 実測 (Runtime-only harness, 780M) |
| **F57** | **旧 build の trace ON/OFF で O11 の外形症状が一致した。** 固定条件 (同一 granite GGUF / プロンプト固定 / WebGPU peer 1 / timeout 300 s) で **OFF 2 回 / ON 1 回**を実行し、3 回とも `ready` 到達 → **first token なし** → **runtime error なし** → 300 s で `generate() timed out after 300s`。peer は全 run で生存し RPC ループを回し続けた。⚠️ OFF には内部 trace が無いので比較できるのは**外形症状だけ**であって「停止位置が同じ」ではない。⚠️ 今回の診断変更後 build での OFF → ON 1 セットは未実施 (**UNTESTED**) | 実測 (780M) |
| **F58** | **Qwen3.6 の実 op を、重みを落とさずに CPU 参照と突き合わせられる。** `export-graph-ops` が `mradermacher/Qwen3.6-35B-A3B-GGUF:Q2_K` のメタデータのみから **136 unique ops** (prefill 73 / decode 63) を書き出し、`test-backend-ops --test-file` が実行した結果は **OK 131 / FAIL 1 / NOT SUPPORTED 4 / expected 136**。**256 experts・n_used=8・実 Q3_K の `MUL_MAT_ID` (`ffn_moe_down`) は prefill・decode とも OK**。⚠️ **`OK 131` は全 operation の合計であり、通常 `MUL_MAT` の 131/131 集計ではない。** また Qwen3.6 の個別 `MUL_MAT_ID` と Granite node157 の通常 `MUL_MAT` は同一モデル・同一 configuration ではない。これは「export された各 op を個別に実行した結果が CPU 参照と一致する」までで、op を繋いだときの buffer alias / queue / 同期 / lifetime は未検証 — O11 はまさにその領域 | 実測 (780M)。詳細 `docs/evidence/o11-780m-trace-2026-08-28.txt` |
...
| **F62** | **O11 の停止候補を `encode node=157/1591 op=MUL_MAT` まで狭めた。** `patches/0005` を「encode の**前**に出力」へ直し `GGML_WEBGPU_TRACE_STRIDE=1` で全ノードを記録すると、旧 trace は **165 行**で停止し3分以上不変。`submit` は 2 回のみ、`wait(final)` 未到達、blocking `WaitAny` trace 行 0。⇒ **この測定では `ggml_webgpu_encode_node()` が node157 の呼び出しから返らない**。⚠️ **前ビルド (encode の後に出力) の最終行が node156 だったのと 1 ずれるのは、ログ位置を1つ前へ動かした分の想定どおり**で、同じ停止を見た傍証に過ぎない。⚠️ node157 が `MUL_MAT` であることから、kernel、graph state、buffer、binding、pipeline cache のいずれかを原因確定はできない。Part D の `OK 131` は全 operation 合計で、通常 `MUL_MAT` の 131/131 ではない。Qwen3.6 の個別 `MUL_MAT_ID` / standalone 数値 pass も、Granite node157 の full graph state / lifetime を検証していない | 実測 (780M)。`docs/evidence/o11-780m-trace-2026-08-28.txt` |
...
| **O11** | **実 MoE モデルの WebGPU peer 実行で first token に到達しない。** CPU peer では同じモデルが 30 秒未満で完走する。**2026-08-28 の旧 trace では停止候補が node157 (`MUL_MAT`) の encode 呼び出しまで狭まった**が、これは原因特定ではない。`wait(final)` 未到達・`submit #2` 未観測である一方、旧 trace の `retries=0` だけから GPU submission 完了は言えず、0-timeout poll の status も未記録だった。⚠️ Part D の `OK 131` は全 operation 合計で、通常 `MUL_MAT` 131/131 ではない。Qwen3.6 の個別 `MUL_MAT_ID` と Granite node157 はモデル / shape / type / path / pipeline が異なり得、patch0004 の standalone 数値 pass も full graph state / lifetime を否定しない。node157 が `MUL_MAT` であることから kernel 以外の graph state 等を原因確定するのも未了。**測定機は AMD Radeon 780M (統合GPU) 1 台のみ** | 変更後 trace の OFF→ON 1 セットが未実施なら追加計測は UNTESTED とし、明確なコード原因が出なければ `docs/L2_TARGET_PC_RUNBOOK.md` の discrete GPU 実験へ進む。⚠️ upstream `a95a11e5b` (#22464) は **効くかは未判定** | **P0** |
