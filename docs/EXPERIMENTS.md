...
閉じても減らず、**peer タブを閉じて初めて**約 740 MiB 落ちる。

⚠️ ここから言えるのは **「peer / WebGPU を生かしている状態に紐づく増分」** までで、
**「peer Runtime の leak」とは断定しない。** 保持している主体 (peer Runtime か、そのタブの
GPU プロセスか、Dawn 側のキャッシュか) は切り分けていない。
