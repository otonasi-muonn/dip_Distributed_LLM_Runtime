...
設計仕様を固定することが目的ではありません。
実装・調査・実測によって前提が変わった場合は、この文書も更新していく想定です。
...
RuntimeをWebアプリ側から独立して利用しやすい形にできるとよさそうですが、npm packageなどの具体的な配布形式はまだ決めていません。
...
`Qwen3.6-35B-A3B` は現在の主要な検証候補です。
モデル選定は、Runtimeの実装状況や実測結果によって変わる可能性があります。
...
独自実装を始める前に、モデル配置・RPC・デバイス管理などについて既存機能で扱える範囲を確認します。
既存機能が今回の用途に合わない場合は、その時点で追加実装や変更を検討します。
...
llmletは独自に固定したllama.cpp revisionを利用しているため、現在使いたいモデルや現行upstreamとの互換性は確認が必要です。
...
このファイルは現在地を共有するための資料として扱います。

作業時は、次の考え方を参考にします。

- ドキュメントより、現在のコード・upstream仕様・実測結果を優先する
- 確認できていないことは、事実と仮説を分けて扱う
- 独自実装の前に、llama.cppやllmletに利用できる仕組みがないか確認する
- 大きな設計を一度に固めるより、小さく動かして確認できる変更を優先する
- 抽象化や最適化は、必要性が見えてから検討する
- 調査やPoCで分かったことは、この文書や関連docsへ反映する
- ハッカソン開発では、まず動作する縦切りのPoCを作ることを重視する

これらは絶対的なルールではなく、迷ったときの判断基準です。
